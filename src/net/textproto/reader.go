...
func canonicalMIMEHeaderKey(a []byte) string {
	...
	return internCommonHeader(a)
}

// commonHeader interns common header strings.
var commonHeaderMu sync.RWMutex
var commonHeader map[string]string

const maxCommonHeaderCount = 512
const maxCommonHeaderLen = 64

func internCommonHeader(s []byte) (is string) {
	if len(s) > maxCommonHeaderLen {
		return string(s)
	}

	if !commonHeaderMu.TryRLock() {
		return string(s)
	}
	var ok bool
	if commonHeader != nil {
		is, ok = commonHeader[string(s)]
	}
	commonHeaderMu.RUnlock()
	if ok {
		return is
	}

	if !commonHeaderMu.TryLock() {
		return string(s)
	}
	defer commonHeaderMu.Unlock()
	if commonHeader == nil {
		time.AfterFunc(1*time.Minute, clearCommonHeader)
		commonHeader = make(map[string]string)
	} else if is, ok := commonHeader[string(s)]; ok {
		return is
	} else if len(commonHeader) >= maxCommonHeaderCount {
		for k := range commonHeader {
			delete(commonHeader, k)
			break
		}
	}
	is = string(s)
...
func clearCommonHeader() {
	commonHeaderMu.Lock()
	commonHeader = nil
	commonHeaderMu.Unlock()
}
