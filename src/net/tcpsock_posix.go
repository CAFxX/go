...
func (c *TCPConn) readFrom(r io.Reader) (int64, error) {
	...
	if n, err, handled := splicefd(c.fd, r); handled {
		return n, err
	}
...
func splicefd(outFD int, r io.Reader) (int64, error, bool) {
...
func _splicefd(inFD, outFD int, max int64) (int64, error, bool) {
	var pFD [2]int
	...
	for unread := max; unread > 0; {
		nr, err := syscall.Splice(inFD, nil, pFD[1], nil, unread, syscall.SPLICE_F_MOVE)
		if err != nil || nr == 0 {
			return max - unread, err, max != unread
		}
		unread -= nr
		for nr > 0 {
			nw, err := syscall.Splice(pFD[0], nil, outFD, nil, nr, syscall.SPLICE_F_MOVE|syscall.SPLICE_F_MORE)
			if err != nil {
