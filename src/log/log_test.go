...
func BenchmarkItoa(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		dst = dst[0:0]
		dst = itoa(dst, 2015, 4)   // year
		dst = itoa(dst, 1, 2)      // month
		dst = itoa(dst, 30, 2)     // day
		dst = itoa(dst, 12, 2)     // hour
		dst = itoa(dst, 56, 2)     // minute
		dst = itoa(dst, 0, 2)      // second
		dst = itoa(dst, 987654, 6) // microsecond
	}
}
